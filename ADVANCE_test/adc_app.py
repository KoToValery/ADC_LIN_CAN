...
INGRESS_PATH = os.getenv('INGRESS_PATH', '')  # <-- Добавено: променлива за Ingress път
...
# Важно: Променяме WebSocket маршрута да използва INGRESS_PATH
...
def apply_ma_then_ema(value, channel, is_voltage):
    ...
    if is_voltage:
        return round(ema_values[channel], 2)
    else:
        return int(round(ema_values[channel]))
